...
class Repository:
    @db_session
    def create_user(self, name, create_stuff=False):
        if create_stuff:
            date_of_hire = datetime.datetime.now()
            user = User(name=name, create_stuff=create_stuff, date_of_hire=date_of_hire)
            return user
        else:
            user = User(name=name)
            return user
    ...
    #         good_obj=Good.select(id=good_dict["id"])[:]._items[0] #TODO Сделать одним запросом
